...
const IncomeList = ({ refreshTrigger, displayCurrency = "USD" }: Props) => {
    ...
    <Box sx={{ mt: 4 }}>
      ...
      <Box sx={{ mb: 2, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <Typography variant="h6">
          Your Income ({filteredTransactions.length})
        </Typography>
        <Typography variant="h6" color="success.main" fontWeight="bold">
          Total: {formatAmount(getTotalIncome(), displayCurrency)}
        </Typography>
      </Box>

      {filteredTransactions.length === 0 ? (
        <Alert severity="info" sx={{ mb: 3 }}>
          No income found for the selected filter.
        </Alert>
      ) : (
